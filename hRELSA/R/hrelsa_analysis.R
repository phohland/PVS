...
hrelsa_analysis <- function(set) {
  ...
  if (abort) {
    ...
    variables  <- names(set[, (which(names(set) == "day") + 1):(length(names(set)) - 2)])
    
    subset <- set[, (which(names(set) == "day") + 1):(length(names(set)) - 2)]
    ...
    if (wf_rms_same) {
      analysis <- list(
        ...
        variables = variables,
        max_severity_of_each_variable = max_wfactor_rdy,
        the_same_patient_has_max_wf_and_rms = wf_rms_same,
        max_severity = max_wf_rdy,
        max_severity_whole_patient = max_wf_pat_rdy
      )
    } else {
      analysis <- list(
        amount_of_unique_patients = n,
        ...
        variables = variables,
        max_severity_of_each_variable = max_wfactor_rdy,
        max_severity_wf = max_wf_rdy,
        max_severity_wf_whole_patient = max_wf_pat_rdy,
        max_severity_rms = max_rms_rdy,
        max_severity_rms_whole_patient = max_rms_pat_rdy
      )
    }
